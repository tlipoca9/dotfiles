...
hypothetical future scale. Use the finding contract to reason, not as a mandatory
field-by-field output template. For public PR comments, lead with the concrete
problem and impact at the responsible line, then give a concise correction
direction. Include reproduction details only when needed to assess the claim;
keep tool logs and the investigation narrative out of the comment. Match depth
to the artifact's intended audience and purpose, including whether it is a user
example, production feature, or CI helper. Posting requires explicit authorization.
