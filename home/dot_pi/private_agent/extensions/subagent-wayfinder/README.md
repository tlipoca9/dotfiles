...
Tracked children use the map's tracker-specific pitfall log as shared operational memory. GitHub and Gongfeng use append-only comments headed `WAYFINDER PITFALL`; TAPD mini uses entries appended under the Map description's `## 可复用障碍` because its MCP comment tools do not accept mini-items. Before commands or diagnosis, each child reads every entry; after an unexpected failure it searches them by symptom and component before another attempt.

A reportable pitfall is narrowly limited to a non-obvious operational obstacle from build, deploy, tooling, permissions, environment, or shared infrastructure that can recur in another independent ticket and be reused without knowing the current ticket's decision history. Normal ticket iteration—hypotheses, rejected options, clarification, feedback, prototype changes, changing understanding, routine trial and error, typos, and transient failures from code under change—stays in the ticket resolution or is omitted. Searching the log after a failure does not make that failure reportable. Qualifying obstacles record the ticket, scope, symptom, cause, resolution, verification, and resolved/unresolved status.
