---
name: tencentcloud-yunapi-gateway-request-id-escalation
description: Escalate opaque external gateway or cloud API failures by extracting request IDs and evidence before debugging. Use when Codex sees YunAPI, TencentCloudSDKError, cloud gateway, API gateway, pre-release gateway, or SDK errors such as InternalError, UnknownParameter, InvalidParameter, BackendUnavailable, or generic "internal service error", especially when backend service logs suggest the request reached or succeeded in the target service.
---
