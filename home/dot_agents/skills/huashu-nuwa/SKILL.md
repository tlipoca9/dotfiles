...
首次扫描、创建或更新Skill前，先解析一次`[skills-source]`，后续全程复用同一目录：

1. 如果当前Git仓库根目录包含`.chezmoiroot`，运行`chezmoi source-path --source <repo-root> ~/.agents/skills`。
2. 否则运行`chezmoi source-path ~/.agents/skills`，使用当前chezmoi源目录中的Skill路径。
3. 如果chezmoi不可用或该目标尚未纳管，回退到`~/.agents/skills`，并在交付时明确说明结果尚未进入可复现的chezmoi源。

不要把用户级Skill写入`~/.codex/skills`或`~/.claude/skills`。如果`[skills-source]`与`~/.agents/skills`不同，完成后提醒用户通过chezmoi预览并应用，而不是再复制一份。

如果调研材料来自私密聊天、个人会话或其它不应进入公开仓库的来源，不要把完成的Skill以明文留在`[skills-source]`。先在`~/.agents/skills/[person-name]-perspective`生成并复核，再用仓库现有的chezmoi加密配置执行`chezmoi add --source <repo-root> --encrypt --recursive`纳管；随后确认源目录只含`encrypted_*.age`文件。更新已经加密的Skill时也走同一流程，不要用明文覆盖加密源。
...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？（检查`[skills-source]`目录）
...
**来源A：本地已有Skill**
扫描`[skills-source]/*-perspective/`目录，读取每个SKILL.md的description，匹配用户需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（用户还没有任何perspective skill），跳过此步，只从来源B推荐。
...
```
[skills-source]/[person-name]-perspective/
├── SKILL.md                          # 最终产物
...
Phase 1启动前，**主动扫描`[skills-source]`目录**，检查是否有可用于信息获取的skill。如果有，在调研中优先调用，比WebSearch更稳定高效：
...
#### Step 4: 输出
将完成的SKILL.md写入`[skills-source]/[person-name]-perspective/SKILL.md`。
