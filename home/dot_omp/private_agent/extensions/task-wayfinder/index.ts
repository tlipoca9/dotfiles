...
An execution-bearing map has one retained map supervisor for its active execution. Read the latest tracker-specific pitfall log before launch. GitHub and Gongfeng use comments headed WAYFINDER PITFALL; TAPD mini uses the map description's ## Pitfall log. Children report reusable failures through hub with [wayfinder:pitfall_report] and ask one unresolved material question through hub with [wayfinder:interview_request]. The parent is the only pitfall-log writer, deduplicates by normalized Scope + Symptom + Cause immediately before append, acknowledges every report, and discloses all new, reused, or unresolved pitfalls at handoff, or states Pitfalls: None.`;
