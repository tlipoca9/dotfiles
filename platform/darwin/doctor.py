...
from zsh_plugins import ManifestError, ZshPluginManifest  # pyright: ignore[reportMissingImports]  # noqa: E402
from pi_config import (  # pyright: ignore[reportMissingImports]  # noqa: E402
    PiConfigError,
    declared_npm_packages,
    load_managed_pi_settings,
)
...
class EnvironmentDoctor:
    ...
    def check_brew(self) -> None:
        ...
        probe = 'eval "$("$1" shellenv)" && test -n "$HOMEBREW_PREFIX" && test -x "$HOMEBREW_PREFIX/bin/brew"'
        self.command(("/bin/sh", "-c", probe, "dotfiles-doctor", brew))
    ...
    def check_brewfile(self) -> None:
        ...
        self.command(
            (
                ...
                "check",
                "--no-upgrade",
                f"--file={self.repo_root / 'platform' / 'darwin' / 'Brewfile'}",
            ),
            timeout=60,
        )
    ...
    def check_pi(self) -> None:
        ...
        lean_ctx = shutil.which("lean-ctx")
        if pi is None or node is None or lean_ctx is None:
            raise DoctorFailure("Pi, Node.js, and lean-ctx must be installed")
        self.command((pi, "--version"))
        self.command((lean_ctx, "--version"))

        settings_path = self.home / ".pi" / "agent" / "settings.json"
        try:
            current = json.loads(settings_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as error:
            raise DoctorFailure(f"cannot read Pi settings: {error}") from error
        if not isinstance(current, dict):
            raise DoctorFailure("Pi settings must contain a JSON object")
        try:
            expected = load_managed_pi_settings(self.repo_root, current)
            packages = declared_npm_packages(expected)
        except PiConfigError as error:
            raise DoctorFailure(str(error)) from error
        if current != expected:
            raise DoctorFailure("managed Pi settings are stale; run task apply")

        package_root = self.home / ".pi" / "agent" / "npm" / "node_modules"
        missing: list[str] = []
        wrong_versions: list[str] = []
        for name, version in packages:
            manifest = package_root.joinpath(*name.split("/"), "package.json")
            try:
            ...
            if not isinstance(installed, dict) or installed.get("version") != version:
                wrong_versions.append(f"{name} (expected {version})")
        if missing:
            raise DoctorFailure(f"managed Pi packages are missing: {missing}")
        if wrong_versions:
            raise DoctorFailure(f"managed Pi package versions differ: {wrong_versions}")
        listed = self.command((pi, "list"), timeout=120)
        load_output = listed.stdout + listed.stderr
        load_failures = [
            line.strip()
            for line in load_output.splitlines()
            if "Failed to load extension" in line or "Failed to start" in line
        ]
        if load_failures:
            raise DoctorFailure(f"Pi extension loading failed: {load_failures}")
